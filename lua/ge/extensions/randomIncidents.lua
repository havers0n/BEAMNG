...
function M.printSceneSummary()
  ...
  local scene = generatedScene
  log('I', logTag, string.format(
    'Scene summary: spot=%s nodes=%s->%s targetPath={%s} posA=(%.2f, %.2f, %.2f) posB=(%.2f, %.2f, %.2f) leadSpeedMps=%.2f chaseSpeedMps=%.2f distanceBehind=%.2f',
    tostring(scene.spotIndex), tostring(scene.nodeA), tostring(scene.nodeB),
    scene.targetPath and table.concat(scene.targetPath, ', ') or '',
    scene.posA.x, scene.posA.y, scene.posA.z,
    scene.posB.x, scene.posB.y, scene.posB.z,
    scene.leadSpeedMps, scene.chaseSpeedMps, scene.distanceBehind
  ))
  return scene
end
