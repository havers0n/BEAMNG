...
local function buildSnapshot(report)
  ...
  local actors, samples, summaries = {}, {}, {}
  ...
  return {sessionId=report.sessionId, scenarioId=report.scenarioId, seed=report.seed, status=report.status, startedAtSceneTime=report.startedAtSceneTime, stoppedAtSceneTime=report.stoppedAtSceneTime, sampleInterval=report.sampleInterval, actorCount=#actors, sampleCount=#samples, droppedSampleCount=tonumber(report.droppedSampleCount) or 0, invalidFieldCount=tonumber(report.invalidFieldCount) or 0, actors=actors, samples=samples, actorSummaries=summaries}
end
...
function M.render(report, metadata)
  ...
  table.insert(lines, json({recordType='summary', formatVersion=FORMAT_VERSION, sessionId=snapshot.sessionId, actorCount=snapshot.actorCount, sampleCount=snapshot.sampleCount, droppedSampleCount=snapshot.droppedSampleCount, invalidFieldCount=snapshot.invalidFieldCount, exportedAt=exportedAt, exportCount=exportCount}))
  ...
  table.insert(txt, ''); table.insert(txt, '4. Export Metadata'); table.insert(txt, 'exportedAt: ' .. exportedAt); table.insert(txt, 'jsonlPath: ' .. tostring(metadata.jsonlPath or '')); table.insert(txt, 'txtPath: ' .. tostring(metadata.txtPath or '')); table.insert(txt, 'writeBackend: sandboxed_io_open'); table.insert(txt, 'formatVersion: ' .. FORMAT_VERSION); table.insert(txt, 'exportCount: ' .. exportCount)
